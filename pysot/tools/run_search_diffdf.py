...
def main():
    ...
    if args.dataset in ['VOT2016', 'VOT2018', 'VOT2019']:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            toc = 0
            pred_bboxes = []
            for idx, (img, gt_bbox) in tqdm(enumerate(video), total=len(video), desc=f'Processing {video.name}'):
                if len(gt_bbox) == 4:
                ...
                if idx == frame_counter:
                    ...
                    '''DiffDf'''
                    outputs = tracker.track_def(img, GAN)
                ...
                if args.vis and idx > frame_counter:
                    cv2.polylines(img, [np.array(gt_bbox, int).reshape((-1, 1, 2))],
                            True, (0, 255, 0), 3)
                    if cfg.MASK.MASK:
                        cv2.polylines(img, [np.array(pred_bbox, int).reshape((-1, 1, 2))],
                                True, (0, 255, 255), 3)
                    else:
            ...
            # save results
            video_path = os.path.join('results', args.dataset, sy,
                    'baseline', video.name)
        ...
        for v_idx, video in enumerate(dataset):
            ...
            if 'VOT2018-LT' == args.dataset:
                ...
                model_path = os.path.join('results', args.dataset, sy)
