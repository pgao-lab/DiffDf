...
class SiamRPNTracker(SiameseTracker):
    ...
    def init_def(self, img, bbox, GAN, save_path=None, name=None):
        z_crop = self.get_z_crop(img, bbox)
    ...
    def init_adv_S(self, img, bbox, GAN, save_path=None, name=None):
        ...
        if save_path != None and name != None:
            ...
            diff_img = tensor2img(diff)
            cv2.imwrite(os.path.join(save_path, name + '_diff.jpg'), diff_img)
    def init_def_S(self, img, bbox, GAN, save_path=None, name=None):
        z_crop = self.get_z_crop(img, bbox)
        '''Adversarial Attack'''
        z_crop_adv = adv_attack_template_S(z_crop, GAN)
    ...
    def track_adv(self, img, GAN, save_path=None, frame_id=None):
        ...
        # output_dict_ori = self.x_crop_2_res(img, x_crop, scale_z)
        if save_path!=None and frame_id!=None:
            '''save'''
            self.save_img(x_crop,x_crop_adv,save_path,frame_id)
        return output_dict
    
    def track_def(self, img, GAN, save_path=None, frame_id=None):
        x_crop, scale_z = self.get_x_crop(img)
        ...
        '''predict'''
        output_dict = self.x_crop_2_res(img, x_t, scale_z)
        # output_dict_ori = self.x_crop_2_res(img, x_crop, scale_z)
        if save_path!=None and frame_id!=None:
            '''save'''
            self.save_img(x_crop,x_crop_adv,save_path,frame_id)
    ...
    def track_supp(self, img, GAN, save_path, frame_id,name=None):
        x_crop, scale_z = self.get_x_crop(img)
        ...
        x_np = x_np.transpose(2, 0, 1).astype(np.float32)
        x_t  = torch.from_numpy(x_np).unsqueeze(0).to('cuda')
        x_t_img = tensor2img(x_t)
        cv2.imwrite(os.path.join(save_path,'def_search_%d.jpg' % frame_id), x_t_img)
        outputs_t = self.model.track(x_t)
        score2 = self._convert_score(outputs_t['cls']) #(25x25x5,)
        heatmap_t = 255.0*np.max(score2.reshape(5, 25, 25), axis=0)#[0,1]
        heatmap_t = cv2.resize(heatmap_t,(255,255),interpolation=cv2.INTER_CUBIC)
        heatmap_t = cv2.applyColorMap(heatmap_t.clip(0,255).astype(np.uint8), cv2.COLORMAP_JET)
        cv2.imwrite(os.path.join(save_path, 'heatmap_def_%d.jpg' % frame_id), heatmap_t)
