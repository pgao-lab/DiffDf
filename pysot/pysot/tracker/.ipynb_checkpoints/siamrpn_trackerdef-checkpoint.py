...
import sys
sys.path.append(r'/hy-tmp/CSA/pysot')
sys.path.append(r'/hy-tmp/CSA/pix2pix')
sys.path.append(r'/hy-tmp/CSA')
import numpy as np
...
import cv2
import os
from denoise_ddpm import denoise_image
from denoise_ddpm_t import denoise_image_t
...
class SiamRPNTracker(SiameseTracker):
    ...
    def save_img(self, tensor_clean, tensor_adv, save_path, frame_id,video_name):
        ## clean x_crop
        img_clean = tensor2img(tensor_clean)
        cv2.imwrite(os.path.join(save_path, '%s_%06d_clean.jpg'% (video_name, frame_id)), img_clean)
        ## adv x_crop
        img_adv = tensor2img(tensor_adv)
        cv2.imwrite(os.path.join(save_path, '%s_%06d_adv.jpg'% (video_name, frame_id)), img_adv)
        ## diff
        # print("成功保存{}到{}",frame_id,save_path)
    ...
    def init_adv(self, img, bbox, GAN, save_path=None, name=None,idx = None):
        z_crop = self.get_z_crop(img, bbox)
        # '''Adversarial Attack'''
        # z_crop_adv = adv_attack_template(z_crop, GAN)

        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
#         z_np = z_crop_adv.cpu().numpy().astype(np.float32)

#         # 调整 x_np 的形状和数据类型
#         if z_np.ndim == 4:  # 如果 z_np 是 4D 数组 (1, 1, 255, 255)
#             z_np = z_np.squeeze(0)  # 去除多余的维度，变为 (255, 255)
            
            
#         z_np = z_np.transpose(1, 2, 0)
        

#         z_np = denoise_image_t(z_np)
        
#         z_np = z_np.transpose(2, 0, 1).astype(np.float32)
        
#         z_t  = torch.from_numpy(z_np).unsqueeze(0).to('cuda')
      ...
      #￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥  
        ...
        #     cv2.imwrite(os.path.join(save_path, name+'_{}_clean.jpg'.format(idx)),z_crop_img)
        #     z_crop_adv_img = tensor2img(z_crop_adv)
        #     cv2.imwrite(os.path.join(save_path, name + '_{}_adv.jpg'.format(idx)), z_crop_adv_img)
        #     diff = (z_crop_adv - z_crop)*10
        #     diff_img = tensor2img(diff)
        #     cv2.imwrite(os.path.join(save_path, name + '_{}_diff.jpg'.format(idx)), diff_img)

    def init_adv_S(self, img, bbox, GAN, save_path=None, name=None,idx = None):
        z_crop = self.get_z_crop(img, bbox)
        '''Adversarial Attack'''
    ...
    #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
        ...
        # z_np = gaussian(z_np, sigma=1)
        z_np = cv2.medianBlur(z_np, ksize=3)
      ...
      #￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥  
        ...
        self.model.template(z_t)
        '''save'''
        if save_path != None and name != None:
            z_crop_img = tensor2img(z_crop)
            cv2.imwrite(os.path.join(save_path, name+'_{}_clean.jpg'.format(idx)),z_crop_img)
            z_crop_adv_img = tensor2img(z_t)
            cv2.imwrite(os.path.join(save_path, name + '_{}_adv.jpg'.format(idx)), z_crop_adv_img)
            diff = (z_t - z_crop)*10
            diff_img = tensor2img(diff)
            cv2.imwrite(os.path.join(save_path, name + '_{}_diff.jpg'.format(idx)), diff_img)
    ...
    def track(self, img):
        ...
        return output_dict
        # self.save_img(x_crop,x_crop_adv,save_path,frame_id,video_name)

    def track_adv(self, img, GAN, save_path=None, frame_id=None ,video_name = None):
        x_crop, scale_z = self.get_x_crop(img)
        
        
        x_crop_np = x_crop.cpu().numpy().astype(np.uint8)
        if x_crop_np.ndim == 4:
            x_crop_np = x_crop_np.squeeze(0)  
        if x_crop_np.dtype == np.float32:  # 如果数据类型是 float32
            x_crop_np = x_crop_np.astype(np.uint8)
        x_crop_np = x_crop_np.transpose(1, 2, 0)
        ...
        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
        x_np = x_crop_adv.cpu().numpy().astype(np.float32)

        # 调整 x_np 的形状和数据类型
        if x_np.ndim == 4:  # 如果 x_np 是 4D 数组 (1, 1, 255, 255)
            x_np = x_np.squeeze(0)  # 去除多余的维度，变为 (255, 255)
            
            
        x_np = x_np.transpose(1, 2, 0)
        

        x_np = denoise_image(x_np)
        
        # x_np = gaussian(x_np, sigma=1)
        # x_np = cv2.medianBlur(x_np, ksize=3)
        
        x_np = x_np.transpose(2, 0, 1).astype(np.float32)
        
        x_t  = torch.from_numpy(x_np).unsqueeze(0).to('cuda')
      ...
      #￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥  
        ...
        if save_path!=None and frame_id!=None:
            '''save'''
            self.save_img(x_crop,x_t,save_path,frame_id,video_name)
        return output_dict
